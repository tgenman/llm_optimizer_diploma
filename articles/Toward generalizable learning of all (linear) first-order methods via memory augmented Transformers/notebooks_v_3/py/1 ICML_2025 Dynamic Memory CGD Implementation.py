...
# Функция attention реализует механизм внимания для линейного трансформера
def attention(P,Q,Z, activation = None):
    # Получаем размерности входного тензора Z:
    B= Z.shape[0]  # B - размер батча
    N = Z.shape[1]-1  # N - длина контекста (без учета запроса)
    d = Z.shape[2]-1  # d - размерность признаков

    # Расширяем матрицу P добавляя нули, чтобы учесть размерность запроса
    P_full =  torch.cat([P,torch.zeros(1,d).to(device)],dim=0)  # Добавляем строку нулей
    P_full =  torch.cat([P_full,torch.zeros(d+1,1).to(device)],dim=1)  # Добавляем столбец нулей
    P_full[d,d] = 1  # Устанавливаем 1 в правом нижнем углу

    # Аналогично расширяем матрицу Q
    Q_full = torch.cat([Q, torch.zeros(1,d).to(device)],dim=0)
    Q_full = torch.cat([Q_full, torch.zeros(d+1,1).to(device)],dim=1)

    # Создаем маску внимания - единичную матрицу с нулем в правом нижнем углу
    A = torch.eye(N+1).to(device)
    A[N,N] = 0  # Запрещаем обращать внимание на себя для последнего токена

    # Вычисляем матрицу внимания через матричное произведение
    Attn = torch.einsum('BNi, ij, BMj -> BNM', (Z,Q_full,Z))

    # Применяем функцию активации если она задана (например softmax или ReLU)
    if activation is not None:
        Attn = activation(Attn)

    # Вычисляем значения (values) через проекцию входа на P
    key = torch.einsum('ij, BNj -> BNi', (P_full,Z))

    # Получаем выход перемножая матрицу внимания, маску и значения
    Output = torch.einsum('BNM,ML, BLi -> BNi', (Attn,A,key))

    # Нормируем выход на длину контекста
    return Output /N
